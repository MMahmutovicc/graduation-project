...
<main>
    <div class="all">
        <a href="{{ route('lekcije',['sekcija_id' => $lekcija->sekcija->id]) }}" class="all-button">Sve Lekcije</a>
    </div>
    <div class="register-box">
        <h1 class="register-heading">Uredi Lekciju</h1>
        @if (Session::has('lekcija_poruka'))
            <div class="alert alert-success" role="alert">{{ Session::get('lekcija_poruka') }}</div>
        @endif
        <form class="register-form" method="POST" enctype="multipart/form-data" wire:submit.prevent="updateLecture()">
            @csrf
            ...
            <div class="register-form_group">
                ...
                <textarea class="register-description" name="explanation" placeholder="Objašnjenje Lekcije" id="explanation" cols="30" rows="10" wire:model='explanation'></textarea>
                @error('explanation')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
            ...
            @foreach ($inputs as $key=>$value)
                ...
                @endif
                    <div class="register-form_group">
                        ...
                        <input type="text" placeholder="Naslov" id="title" name="title" wire:model="example_values.{{ $key }}.{{ 'title' }}">
                    </div>
                    <div class="register-form_group">
                        <label for="photo">Slika Primjera</label>
                        <input type="file" name="photo" id="photo" wire:model="example_values.{{ $key }}.{{ 'newphoto' }}">
                    </div>
                    @if ($example_values[$key]['newphoto'])
                        @if ($example_values[$key]['newphoto']->extension() == 'jpg' || $example_values[$key]['newphoto']->extension() == 'png')
                            <img src="{{ $example_values[$key]['newphoto']->temporaryUrl() }}" alt="" width="120">
                        @endif
                    @elseif($example_values[$key]['photo'])
                        <img src="{{ asset("storage/examples"."/".$example_values[$key]['photo']) }}" alt="" width="120">
                    @endif
                    <div class="register-form_group">
                        <label for="explanation">Objašnjenje Primjera</label>
                        <textarea class="register-description" name="explanation" placeholder="Objašnjenje Lekcije" id="explanation" cols="30" rows="10"  wire:model="example_values.{{ $key }}.{{ 'explanation' }}"></textarea>
                    </div>
                    @if ($value == 1)  
                        <div class="register-form_group">
                            <label for="input">Tačan Unos</label>
                            <input type="text" placeholder="" id="input" name="input" wire:model="example_values.{{ $key }}.{{ 'input' }}">
                        </div>
                    @endif
            @endforeach
            <div class="register-form_group">
